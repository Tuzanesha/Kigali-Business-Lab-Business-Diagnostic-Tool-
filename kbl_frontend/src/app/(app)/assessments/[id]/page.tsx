...
import { apiCategories, apiQuestions, apiEnterpriseBulkAnswers, apiEnterpriseRecompute, apiEnterpriseReport, apiEnterpriseProfileGet } from '../../../../lib/api';
    ...
    const load = async () => {
      ...
      try {
        ...
        for (const name of names) {
          const qres = await apiQuestions(access, { category: name });
  ...
  const handleNext = () => {
    if (currentStepIndex < steps.length - 1) {
      ...
      const submit = async () => {
        ...
        await apiEnterpriseBulkAnswers(access, enterpriseId, payload);
        await apiEnterpriseRecompute(access, enterpriseId);
        // Optionally fetch report to confirm
        try { await apiEnterpriseReport(access, enterpriseId); } catch {}
      };
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const access = localStorage.getItem('access');
        if (!access) throw new Error('Unauthorized');
        const rep = await apiEnterpriseReport(access, enterpriseId);
  ...
  useEffect(() => {
    const resolve = async () => {
      ...
      if (pid && !isNaN(Number(pid))) {
        ...
        if (access) {
          try {
            const prof = await apiEnterpriseProfileGet(access);
            eid = Number(prof?.id);
          } catch {}
        }
      }
      if (eid) {
        ...
        if (!(pid && !isNaN(Number(pid)))) {
          ...
          try {
            const access = localStorage.getItem('access');
            if (access) {
              await apiEnterpriseReport(access, eid);
              setView('report');
            }
          } catch {
            setView('assessment');
