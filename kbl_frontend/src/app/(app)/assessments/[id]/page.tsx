...
const AssessmentWizard = ({ onComplete, onExit }: { onComplete: () => void, onExit: () => void }) => {
  ...
  const handleNext = () => {
    if (currentStepIndex < steps.length - 1) {
      ...
      const completionPromise = new Promise<void>(resolve => setTimeout(resolve, 1000));
      toast.promise(completionPromise, {
        loading: 'Finalizing assessment...',
        success: 'Report generated successfully!',
        error: 'Could not complete assessment.'
      });
      completionPromise.then(() => onComplete());
    }
  };
...
const AssessmentReport = ({ onRetake }: AssessmentReportProps) => {
  ...
  const handleAddToActionPlan = (itemName: string) => {
    const navigationPromise = new Promise<void>(resolve => setTimeout(resolve, 1000));
    toast.promise(navigationPromise, {
        loading: `Adding ${itemName} to Action Plan...`,
        success: `Redirecting...`,
  ...
  return (
    <div className={styles['report-page']}>
      <header className={styles['report-header']}>
        <div className={styles['report-title-section']}><h1>ASSESSMENT REPORT</h1><p>For Acme Dynamics Inc. - Completed on Wed, 15 Oct 2024</p></div>
      ...
      <div className={styles['report-grid']}>
        <div className={styles['report-card']}><h2 className={styles['section-header']}>Overall Health Score</h2><p className={styles['health-score-value']}>72%</p><p className={styles['health-score-text']}>This score indicates a solid foundation with key areas for strategic improvement.</p></div>
        <div className={styles['report-card']}><h2 className={styles['section-header']}>Performance Overview</h2><div className={styles['overview-chart']}><div className={styles['overview-placeholder']}>72%</div></div></div>
      </div>
      <div className={`${styles['report-card']} ${styles['grid-col-span-2']}`}>
        ...
        <div>
            <div className={styles['focus-item']}>
                <p className={styles['focus-item-title']}>Financials <span className={`${styles['priority-tag']} ${styles['priority-new']}`}>NEW</span></p>
                <button onClick={() => handleAddToActionPlan('Financials')} className={`${styles['nav-button-primary']} ${styles['nav-button']}`}>Add to Action Plan</button>
            </div>
            <div className={styles['focus-item']}>
                <p className={styles['focus-item-title']}>Sales <span className={`${styles['priority-tag']} ${styles['priority-new']}`}>NEW</span></p>
                <button onClick={() => handleAddToActionPlan('Sales')} className={`${styles['nav-button-primary']} ${styles['nav-button']}`}>Add to Action Plan</button>
            </div>
        </div>
      </div>
      <div className={`${styles['report-card']} ${styles['grid-col-span-2']}`}>
        <h2 className={styles['section-header']}>CATEGORY DEEP DIVE</h2>
        <CategoryItem title="Leadership" score={85}><div className={styles['detail-item']}><p className={styles['detail-question']}>Q: How effectively does leadership communicate vision?</p><p className={styles['detail-answer']}>Score: 4/4 (Excellent)</p></div></CategoryItem>
        <CategoryItem title="Sales" score={45}><div className={styles['detail-item']}><p className={styles['detail-question']}>Q: Is the sales process well-documented?</p><p className={styles['detail-answer']}>Score: 2/4 (Some)</p></div></CategoryItem>
      </div>
